...
//        System.exit(0);
		...
		if(result == JOptionPane.YES_OPTION)
		{
			...
			try {				
				...
				// TODO add this functionality to the scheduled section
				...
				//mg2.setKeywords(new MediaKeywords());
				/*
				String quotedkeywords3 = "";
           		for(int i=0; i<quotedkeywords2.length; i++) {
           			quotedkeywords3 = "\"" + quotedkeywords2 + "\"";
					createdEntry.getMediaGroup().getKeywords().addKeyword((String) quotedkeywords3);
    				//quotedkeywords = quotedkeywords + quotedkeywords2[i];
    			}
    			*/ //TODO add ability to use quotes
		        for(int i=0;i<arr.length;i++) {
					createdEntry.getMediaGroup().getKeywords().addKeyword((String) arr[i]);
		        	//mg.getKeywords().addKeyword(arr[i]);
...
// Find out if the file exists already
        	if( fleExample.exists() ) {
        		...
        		while( opnScanner.hasNext() ) {
        			...
        			/*
        			if (privacyoption4 == true) {
            			System.out.println("Privacy option read as true");
        			} else if (privacyoption4 == false) {
            			System.out.println("Privacy option read as false");
        			}
        			System.exit(0);
    		...
    		pwInput.println(filestring);
    		pwInput.println(YTcatorgories3);
    		pwInput.println(privacyoption3);
    		pwInput.print(keywords2);
        			 */
    			...
    			*/
    			...
    			YouTubeMediaGroup mg = newEntry.getOrCreateMediaGroup();
    			mg.setTitle(new MediaTitle());
    			mg.getTitle().setPlainTextContent(desiredtitle2);
    			mg.addCategory(new MediaCategory(YouTubeNamespace.CATEGORY_SCHEME, YTcatorgories3));
    			mg.setKeywords(new MediaKeywords());
    	        for(int i=0;i<arr.length;i++) {mg.getKeywords().addKeyword(arr[i]);
    	        }
    			mg.setDescription(new MediaDescription());
    			mg.getDescription().setPlainTextContent(discription2);
    			//if (privacyoption3 == "Public\n") {
